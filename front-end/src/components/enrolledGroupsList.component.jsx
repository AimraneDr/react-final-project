 ...
 * @returns 
 */
export default function EnrolledGroupsList(props){
    const {module} = props;
    const branches = useSelector(state => state.branches.branches)

    const mBranches = branches.filter(b => b.modules.includes(module.id))
    return (
        <div className="p-5 w-[90%]">
            ...
            <ul className="flex justify-center gap-12 flex-wrap p-6 ">
                {
                    mBranches.map(
                        b => <BranchGroupsStateDropDownList branch={b} module={module} />
                    )
                }
            </ul>
        </div>
...
function BranchGroupsStateDropDownList(props){
    ...
    const groupsList = groups.filter(g => g.branchID === branch.id)
    return(
